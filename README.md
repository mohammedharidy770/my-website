[Uploading nazem_mobile_friendly.html…]()
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1.0,viewport-fit=cover" />
<title>Time Map — mobile</title>
...
body{font-family:'Cairo',sans-serif;background:linear-gradient(180deg,var(--bg),#05020a);color:#eceff8;padding:12px; -webkit-font-smoothing:antialiased}
header{text-align:center;margin-bottom:12px}
h1{font-size:20px;margin-bottom:6px}
.lead{color:var(--muted);font-size:13px;margin-bottom:8px}
.app{max-width:1200px;margin:0 auto;display:flex;gap:var(--gap);align-items:flex-start}
.main{flex:1}
.card{background:var(--card);border-radius:12px;padding:14px;box-shadow:0 8px 30px rgba(0,0,0,0.6)}
.controls{display:flex;gap:8px;align-items:center;margin-bottom:10px;flex-wrap:wrap}
.btn{padding:10px 12px;border-radius:10px;background:transparent;border:1px solid rgba(255,255,255,0.04);cursor:pointer;color:inherit;font-size:15px}
...
.slot{padding:12px;border-radius:10px;border:1px solid rgba(255,255,255,0.03);display:flex;justify-content:space-between;align-items:center;transition:background 0.3s ease,color 0.3s ease;font-size:15px}
.slot .left{display:flex;flex-direction:column;align-items:flex-end}
.slot .left strong{font-size:16px}
...
input,textarea,select{width:100%;padding:10px;border-radius:8px;background:#0a0710;border:1px solid rgba(255,255,255,0.03);color:inherit;margin-bottom:10px;font-size:15px}
...
@media (max-width:900px){
  ...
  .slot{padding:14px;border-radius:12px}
  h1{font-size:18px}
}
@media (max-width:600px){
  ...
  .timeline{gap:12px}
  .slot{font-size:16px;padding:14px}
  .slot .left strong{font-size:17px}
  .summary{font-size:13px}
  aside .card{padding:12px}
  /* make inputs easier to tap */
  input,textarea,select{padding:12px;font-size:16px}
  /* stack control buttons for reachability */
  .controls{flex-direction:column;align-items:stretch}
  .controls-right{margin-left:0;display:flex;gap:8px;justify-content:space-between}
}
...
<body>
<header>
  <h1>نظّم يومك — ابدأ الجدول الآن</h1>
  <div class="lead">اضغط "ابدأ اليوم" والجدول سيبدأ من الوقت الحالي لمدة 8 ساعات مذاكرة (موزعة تلقائيًا). مواقيت الصلاة مدمجة وديناميكية. تمّت إضافة حفظ/استعادة، تصدير/استيراد، مؤقت_SLOT، وإشعارات سطح المكتب.</div>
</header>

<div class="app">
  <main class="main">
    <div class="card" role="region" aria-label="منطقة الجدول الرئيسي">
      <div class="controls" role="toolbar" aria-label="أدوات التحكم">
        <button class="btn primary" id="startBtn">▶️ ابدأ اليوم</button>
        <button class="btn" id="regenBtn">🔁 أعد توزيع</button>
        <div class="controls-right">
          <button class="btn" id="saveScheduleBtn">💾 حفظ الجدول</button>
          <button class="btn" id="loadScheduleBtn">📂 تحميل الجدول</button>
          <button class="btn" id="exportBtn">⬇️ تصدير JSON</button>
          <input id="importFile" type="file" accept="application/json" style="display:none">
        </div>
        <div class="small" id="status" style="margin-inline-start:auto">جاهز</div>
      </div>

      <div id="notice" class="notice"></div>

      <div id="timeline" class="timeline">
        <div style="color:var(--muted);text-align:center;padding:16px">لم تبدأ اليوم بعد — اضغط "ابدأ اليوم" لبدء الجدول من الوقت الحالي.</div>
      </div>

      <div class="timerBar" title="تقدم المذاكرة"><div id="timerFill" class="timerFill"></div></div>
      <div style="display:flex;flex-direction:row;justify-content:space-between;align-items:center;margin-top:12px;gap:12px">
        <div class="summary" id="todaySummary" style="margin-top:12px;flex:1">ملخص اليوم: — لم يبدأ بعد</div>
        <div style="text-align:right;color:var(--muted);font-size:13px;min-width:130px">
          <div id="currentSlotLabel">لا يوجد شغل الآن</div>
  ...
  <aside>
    <div class="card" role="region" aria-label="الإعدادات العامة">
      <h4>الإعدادات العامة</h4>
      <label class="label">مدة الصلاة (دقيقة)</label>
      <input type="number" id="prayerDuration" value="30" inputmode="numeric">
      <label class="label">مدة الاستراحة الافتراضية بين المواد (دقيقة)</label>
      <input type="number" id="breakMinutes" value="15" inputmode="numeric">
      <label class="label">المدينة لمواقيت الصلاة</label>
      <input type="text" id="cityInput" value="Cairo" autocomplete="on">
      <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">
        <button class="btn primary" id="applyBtn">تطبيق الإعدادات</button>
        <button class="btn" id="notifyPermBtn">🔔 طلب إذن إشعارات</button>
      </div>
      <div class="summary" style="margin-top:8px">حفظ الجدول يسمح باستعادة اليوم حتى بعد إعادة تحميل الصفحة. الإشعارات تظهر على سطح المكتب إن سمحت المتصفح.</div>
    </div>

    <div class="card" style="margin-top:12px" role="region" aria-label="المواعيد">
      <h4>مواعيدي — اكتبها هنا</h4>
      <label class="label">اكتب مواعيدك أو ملاحظات التذكير</label>
      <textarea id="appointments" placeholder="مثال: موعد مع الدكتور الخميس 6 مساءً"></textarea>
      <button class="btn primary" id="saveAppointments">💾 احفظ المواعيد</button>
      <div class="summary" style="margin-top:8px">المواعيد تُحفظ محليًا في متصفحك وسيتم عرضها داخل التذكير بعد الانتهاء.</div>
    </div>

    <div class="card" style="margin-top:12px" id="swapSubjectsCard" role="region" aria-label="تبديل المواد">
      <h4>تبديل المواد في الجدول</h4>
      <label class="label">اختر رقم المادة الأولى</label>
      <input type="number" id="swapIndex1" min="1" placeholder="مثال: 1" inputmode="numeric">
      <label class="label">اختر رقم المادة الثانية</label>
      <input type="number" id="swapIndex2" min="1" placeholder="مثال: 5" inputmode="numeric">
      <label class="label">اختر المادة الجديدة من الـ16 مادة</label>
      <select id="newSubjectSelect">
        <option value="">اختر مادة</option>
        <option value="رياضة">قرآن</option>
        <option value="إنجليزي">إنجليزي</option>
        <option value="فيزياء">فيزياء</option>
        <option value="كيمياء">كيمياء</option>
        <option value="أحياء">أحياء</option>
        <option value="قرآن">جولوجيا</option>
        <option value="حديث">عربي</option>
      </select>
      <button class="btn primary" id="swapBtn">🔄 تبديل / تحديث المادة</button>
      <div class="summary" style="margin-top:8px">يمكنك تبديل أو تحديث أي مادة في الجدول بالنقر على "تبديل / تحديث المادة". اختصارات: Space لتعليم/إلغاء، R لإعادة توزيع.</div>
    </div>
  </aside>
</div>
...
<div id="modalBackdrop" class="modal-backdrop" role="dialog" aria-modal="true">
  <div class="modal" role="document">
    <h3>انتهت 8 ساعات المذاكرة — تذكيرات اليوم</h3>
    <div id="modalContent" class="content">جارٍ التحميل...</div>
    <div class="actions">
...
<script>
// === kept JS logic, minor mobile-friendly tweaks only ===
const SUBJECT_POOL = [
  {name:'أحياء', dur:120},{name:'إنجليزي', dur:120},{name:'فيزياء', dur:120},{name:'كيمياء', dur:120},{name:'أحياء', dur:120},
  {name:'قرآن', dur:60},{name: 'عربي', dur:120}
];
...
function saveAppointments(){ localStorage.setItem(APPOINT_KEY, appointmentsInput.value); alert('تم حفظ مواعيدك محليًا'); }
saveAppointmentsBtn.addEventListener('click', saveAppointments);
loadAppointments();

function saveScheduleToStorage(schedule){ localStorage.setItem(SCHEDULE_KEY, JSON.stringify(schedule)); alert('تم حفظ الجدول محليًا'); }
function loadScheduleFromStorage(){ const raw = localStorage.getItem(SCHEDULE_KEY); if(!raw) return null; try{ return JSON.parse(raw);}catch(e){console.warn('خطأ في قراءة الجدول من التخزين',e); return null;} }

saveScheduleBtn.addEventListener('click', ()=>{ if(!currentSchedule) return alert('لا يوجد جدول لحفظه'); saveScheduleToStorage(currentSchedule); });
loadScheduleBtn.addEventListener('click', ()=>{ const loaded = loadScheduleFromStorage(); if(!loaded) return alert('لا يوجد جدول محفوظ'); applyLoadedSchedule(loaded); alert('تم تحميل الجدول من التخزين'); });
...
importFile.addEventListener('change', (e)=>{ const f = e.target.files[0]; if(!f) return; const reader = new FileReader(); reader.onload = ()=>{ try{ const parsed = JSON.parse(reader.result); applyLoadedSchedule(parsed); alert('تم استيراد الجدول بنجاح'); }catch(err){alert('ملف غير صالح');} }; reader.readAsText(f); });

notifyPermBtn.addEventListener('click', async ()=>{
  if(!('Notification' in window)) return alert('المتصفح لا يدعم إشعارات سطح المكتب');
  const perm = await Notification.requestPermission(); alert('حالة الإشعارات: '+perm);
});
...
function updateSummaries(){
  ...
  todaySummaryEl.innerText = `ملخص اليوم: ${done} من ${studySlots.length} مادة تم إنجازها`;
}
...
function triggerPrayerAlert(prayerName){
  ...
  noticeEl.innerText = `🕌 حان الآن موعد صلاة ${prayerName}`;
  noticeEl.style.display = 'block';
  notifyUser('موعد صلاة', `حان الآن موعد صلاة ${prayerName}`);
...
async function startDay(){
  ...
  statusEl.innerText = 'جاري التحضير...';
  ...
  updateSummaries();
  statusEl.innerText = `اليوم جاري — مواقيت من: ${city}`;
...
function startSlotTimer(){
  ...
  function update(){
    ...
    if(current){
      currentSlotLabel.innerText = `${current.type==='study'? current.subject : (current.type==='prayer'? current.name : 'استراحة')} — ${toTime(current.start)}-${toTime(current.finish)}`;
...
swapBtn.addEventListener('click', ()=>{
  ...
  const studySlots = currentSchedule.studySlots.filter(s=>s.type==='study');
  if(isNaN(idx1) || idx1<0 || idx1>=studySlots.length) return alert('أدخل رقم مادة صحيحة');
  if(idx2!==-1 && (isNaN(idx2) || idx2<0 || idx2>=studySlots.length)) return alert('أدخل رقم مادة ثانية صحيحة');
  ...
  const now = new Date(); headerNote.innerText=`انطلق من: ${pad(now.getHours())}:${pad(now.getMinutes())} — مدة المذاكرة الإجمالية: 8 ساعات`;
  timelineEl.appendChild(headerNote);
  ...
  updateSummaries();
  alert('تم تحديث / تبديل المادة بنجاح!');
});
...
regenBtn.addEventListener('click', async ()=>{
  ...
  const heavyNames = ['إنجليزي','رياضيات','فيزياء','أحياء'];
  ...
  alert('تم إعادة توزيع المواد اليومية بنجاح! مجموع المذاكرة 8 ساعات مع Animation والمدد الصحيحة.');
});

applyBtn.addEventListener('click',()=>{ startDay(); alert('تم تطبيق الإعدادات'); });
...
function applyLoadedSchedule(loaded){
  if(!loaded || !loaded.studySlots) return alert('الملف لا يحتوي على جدول صالح');
  currentSchedule = loaded;
  timelineEl.innerHTML = '';
  const headerNote = document.createElement('div'); headerNote.style.color='var(--muted)'; headerNote.style.textAlign='center'; headerNote.style.padding='6px 0';
  const now = new Date(); headerNote.innerText=`انطلق من: ${pad(now.getHours())}:${pad(now.getMinutes())} — جدول مستعاد`;
  timelineEl.appendChild(headerNote);
...
window.addEventListener('keydown', (e)=>{
  if(e.code === 'Space'){ e.preventDefault(); const firstStudy = timelineEl.querySelector('.slot.study'); if(firstStudy) { firstStudy.classList.toggle('done'); updateSummaries(); } }
  if(e.key.toLowerCase() === 'r'){ regenBtn.click(); }
});

window.addEventListener('load', ()=>{
  const saved = loadScheduleFromStorage(); if(saved){ statusEl.innerText = 'جدول محفوظ موجود — اضغط "تحميل الجدول" لاستعادته.'; }
});
